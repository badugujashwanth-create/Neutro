...
function normalizeBundle(bundle: ReplayBundle): ReplayBundle {
  return {
    ...bundle,
...
export async function listReplayBundles(): Promise<ReplayBundle[]> {
  ...
  try {
    return await withStore('readonly', async (store) => {
      ...
      return sortByNewest(bundles.map(normalizeBundle))
    })
  } catch {
    return sortByNewest(readFallback().map(normalizeBundle))
  }
}

export async function getReplayBundle(id: string): Promise<ReplayBundle | null> {
  if (!hasIndexedDb) {
    const bundle = readFallback().find((item) => item.id === id)
    return bundle ? normalizeBundle(bundle) : null
  }
